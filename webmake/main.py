...
def command_line_error(parser, makefile, message):
    ...
    if tb:
        try:
            tb = [s for s in reversed(traceback.extract_tb(tb)) if s[0].endswith("webmakefile.py")][
                0
            ]
...
def parse_command_line():
    ...
    parser.add_argument(
        ...
        "--release",
        action="store_true",
        help="Release mode. Minify output and don't generate source maps.",
    )
    parser.add_argument(
        "-f",
        "--force",
        action="store_true",
        help="Force recompilation of all source files, even if not modified.",
    )
    parser.add_argument(
        "-m",
        "--makefile",
        default=default_webmakefile,
        help="Specify the webmakefile.py to use for compilation settings.",
    )
    parser.add_argument(
        "-w",
        "--watch",
        action="store_true",
        help="Watch all input files for changes, and recompile automatically.",
    )
    parser.add_argument(
        "-p",
        "--polling-watcher",
        action="store_true",
        help="Use the polling watcher instead of inotify (increased compatibility).",
    )
    ...
    if not args.makefile or not os.path.isfile(args.makefile):
        error_fn(
            "webmakefile.py does not exist, please create one in "
...
def main():
    ...
    if settings.FORCE:
        ...
        if not compiler.compile_if_modified(
            settings.MAKEFILE, settings.MAKEFILEPATH, release=settings.RELEASE
        ):
            sys.exit(1)
