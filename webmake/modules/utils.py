...
def resolve_possible_paths(
    path, relative_prefix, possible_extensions=None, leading_underscore=False
):
    """
    ...
    possible_paths = [
        path + e if os.path.isabs(path + e) else os.path.join(relative_prefix, path + e)
        for e in possible_extensions
    ]

    if leading_underscore and not os.path.basename(path).startswith("_"):
        extra_paths = [
            os.path.join(os.path.dirname(p), "_" + os.path.basename(p)) for p in possible_paths
        ]
...
def run_command(cmd, errmsg, env=None, with_node=True):
    ...
    try:
        logv(">>> " + cmd)
        return subprocess.check_output(cmd, env=env, stderr=subprocess.STDOUT, shell=True).decode(
            "ascii", "replace"
        )
...
def extract_lines_from_source(source, line_num):
    ...
    ret = ["{}: {}".format(i + start + 1, line) for (i, line) in enumerate(lines)]
    ret[3] = ret[3].rstrip() + "      <<<<<<\n"
    return "".join(ret)
